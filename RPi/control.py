...
class Side(threading.Thread):
    def __init__(self, port, data, lock, wlock, rate=115200):
        threading.Thread.__init__(self)
        self.data = data
        self.seq = []
        self.lock = lock
    ...
    def main(self, seq):
        ...
        while True:
            ...
            self.lock.release()
            if len(self.data) >= len(self.seq):
                self.makemessage(self.data, seq, ardmsg)
...
def main():
    ...
    # connectedFlag = []
    data = []
    datalock = multiprocessing.RLock()
    writelock = multiprocessing.Lock()
    usbports = []
    if sys.platform.startswith("win"):
        import serial.tools.list_ports
        usbports = list(serial.tools.list_ports.comports())
        for i in usbports : i = i[0][0]
    elif sys.platform.startswith("linux"):
        import glob
        usbports = glob.glob("/dev/ttyACM*")
    arduinos = []

    for arduino in usbports:
        connectedFlag.append([])
        arduinos.append(Side(arduino[0], data, datalock,writelock, connectedFlag[-1]))
    ...
    bp = multiprocessing.Process(target=b)
    bp.start()
